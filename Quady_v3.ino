...
LiquidCrystal_I2C LCD(0x38, 16, 2); // Инициализация LCD дисплея с адресом 0x38 и размером 16x2
...
void generiruyKody() {
  ...
  for (int r = 0; r < a; r++) {
    for (int i = 0; i < a; i++) {
      ...
      Ver[r][i] = randNumber; // Заполнение массива случайными числами от 0 до 9
    }
  }
...
void setup() {
  ...
  generiruyKody(); // Генерация кодов только один раз при старте
